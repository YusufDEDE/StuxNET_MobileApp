...
export default class PastPaymentPopup extends Component {
  state = {
    data: null,
  };
  ...
  componentDidMount() {
    ...
    Api.Auth.paymentTransactionsPopup({transID: item.transID})
      .then(res => {
        console.log('iii', res);
        this.setState({
          data: res[0],
    ...
    return (
      <BaseModal
        ...
        <View style={styles.topContainer}>
          {data ? (
            <>
              <Text style={styles.title}>Fatura Türü: {data.BILLTYPE}</Text>
